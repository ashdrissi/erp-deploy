(function () {
    ...
    const STATE = {
        ...
        business_type: "All",
        segment: "All",
        delivery_progress: "All",
        billing_progress: "All",
    ...
    frappe.pages["sales-order-pipeline"].on_page_load = function (wrapper) {
        const page = frappe.ui.make_app_page({ parent: wrapper, title: __("Sales Order Pipeline"), single_column: true });
        wrapper.page = page;
        page.main.addClass("olp-root olp-sales-root");
        injectPipelineStyles();
        injectSalesOrderPipelineFilterStyles();
        render(page);
        load(page);
        header(page);
    };

    frappe.pages["sales-order-pipeline"].on_page_show = function (wrapper) {
        if (!wrapper.page) return;
        wrapper.page.main.addClass("olp-root olp-sales-root");
        injectPipelineStyles();
        injectSalesOrderPipelineFilterStyles();
        header(wrapper.page);
        load(wrapper.page);
    ...
    function header(page) {
        page.set_title(__("Sales Order Pipeline"));
        setTimeout(() => {
            if (!frappe.breadcrumbs) return;
            frappe.breadcrumbs.clear();
            frappe.breadcrumbs.append_breadcrumb_element("/app/crm-dashboard", __("CRM"), "title-text");
            frappe.breadcrumbs.append_breadcrumb_element("", __("Sales Order Pipeline"), "title-text");
            frappe.breadcrumbs.toggle(true);
        }, 0);
    }

    async function load(page, restore = {}) {
        try {
            const res = await frappe.call({
                method: "orderlift.orderlift_crm.api.pipeline.get_sales_order_pipeline_data",
                args: {
                    search: STATE.search,
                    ...
                    billing_progress: STATE.billing_progress,
                },
            });
    ...
    function render(page, hasError = false, restore = {}) {
        ...
        page.main.html(`
            <div class="olp-shell">
                ...
                <section class="olp-toolbar">
                    <div class="olp-filter-row olp-filter-row--sales-primary">
                        ${filterInput("olp-search", __("Search"), STATE.search, __("Sales order, customer, company"), "search")}
                        ...
                        <div class="olp-filter-actions">
                            <button data-create-sales-order="1">${__("Create Sales Order")}</button>
                            <button data-toggle-advanced="1">${STATE.advancedFiltersOpen ? __("Hide Filters") : __("More Filters")}</button>
                    ...
                    ${STATE.advancedFiltersOpen ? `<div class="olp-filter-row olp-filter-row--sales-advanced">
                        ...
                        ${filterCombo("olp-delivery", __("Delivery"), STATE.delivery_progress === "All" ? "" : STATE.delivery_progress, __("All delivery"), STATE.filters.delivery_progress || [])}
                        ${filterCombo("olp-billing", __("Billing"), STATE.billing_progress === "All" ? "" : STATE.billing_progress, __("All billing"), STATE.filters.billing_progress || [])}
                    </div>` : ""}
                    ${filterChipBar()}
                </section>
                ...
                <section class="olp-board">
                    ${STATE.columns.length ? STATE.columns.map(columnMarkup).join("") : `<div class="olp-empty-board">${hasError ? __("Unable to load the Sales Order pipeline right now.") : __("No sales order statuses or orders are available yet.")}</div>`}
                </section>
                ${totalCards ? "" : `<div class="olp-empty-note">${__("There are currently no sales orders matching the selected filters.")}</div>`}
            </div>
        `);
        bind(page);
    ...
    function bind(page) {
        ...
        page.main.find("[data-create-sales-order]").on("click", createSalesOrder);
        ...
        bindCombo(page, "#olp-delivery", "delivery_progress");
        bindCombo(page, "#olp-billing", "billing_progress");
        page.main.find("[data-toggle-advanced]").on("click", function () {
            STATE.advancedFiltersOpen = !STATE.advancedFiltersOpen;
            render(page, false, { boardScrollLeft: getBoardScrollLeft(page) });
        });
        ...
        page.main.find("[data-create-from-sales-order]").on("click", function (event) {
            event.preventDefault();
            event.stopPropagation();
            createFromSalesOrder($(this).attr("data-create-from-sales-order"), $(this).attr("data-card"));
        });
        page.main.find(".olp-card").on("click", function (event) {
            if ($(event.target).closest("[data-toggle-card], [data-assign-card], [data-create-from-sales-order]").length) return;
            if ($(event.target).closest(".olp-docs-section").length) return;
            frappe.set_route("Form", "Sales Order", $(this).data("name"));
        });
        ...
        page.main.find(".olp-column[data-column!='__unassigned__']").on("drop", async function (event) {
            ...
            if (targetColumn && targetColumn.confirmation_message) {
                showStageMoveConfirmationPopup(page, {
                    documentLabel: __("Sales Order"),
                    record: name,
                    stage,
                    message: targetColumn.confirmation_message,
                    boardScrollLeft,
                    onConfirm: () => moveSalesOrderCard(page, name, stage, boardScrollLeft),
                });
                STATE.dragged = null;
                return;
            }
            await moveSalesOrderCard(page, name, stage, boardScrollLeft);
            STATE.dragged = null;
        });
    ...
    async function moveSalesOrderCard(page, name, stage, boardScrollLeft) {
        try {
            ...
            load(page, { boardScrollLeft });
        } catch (error) {
            const message = extractServerMessage(error) || __("This sales order cannot move to {0} yet.", [stage]);
            showStageMoveErrorPopup(page, { documentLabel: __("Sales Order"), record: name, stage, message, boardScrollLeft });
        } finally {
            STATE.dragged = null;
        }
    }
    ...
    function createSalesOrder() {
        ...
        if (STATE.status !== "All") routeOptions.custom_orderlift_order_status = STATE.status;
    ...
    function filterChipBar() {
        ...
        return `<div class="olp-active-filters" aria-label="${frappe.utils.escape_html(__("Active filters"))}">${chips.map((chip) => `<button type="button" data-clear-filter="${frappe.utils.escape_html(chip.key)}"><span>${frappe.utils.escape_html(chip.label)}</span>${frappe.utils.escape_html(chip.value)}<strong>x</strong></button>`).join("")}</div>`;
    }
    ...
    function activeFilterChips() {
        return [
            ...
            STATE.status !== "All" ? { key: "status", label: __("Status"), value: STATE.status } : null,
            STATE.business_type !== "All" ? { key: "business_type", label: __("Type"), value: STATE.business_type } : null,
            STATE.segment !== "All" ? { key: "segment", label: __("CRM segment"), value: STATE.segment } : null,
            STATE.delivery_progress !== "All" ? { key: "delivery_progress", label: __("Delivery"), value: STATE.delivery_progress } : null,
            STATE.billing_progress !== "All" ? { key: "billing_progress", label: __("Billing"), value: STATE.billing_progress } : null,
        ].filter(Boolean);
    }
    ...
    function resetFilters() {
        ...
        STATE.owner = "All";
        STATE.status = "All";
        STATE.business_type = "All";
        STATE.segment = "All";
        STATE.delivery_progress = "All";
        STATE.billing_progress = "All";
    }
    ...
    function updateSalesOrderStage(salesOrder, stage) {
        return new Promise((resolve, reject) => {
            frappe.call({
                method: "orderlift.orderlift_crm.api.pipeline.update_sales_order_stage",
                args: { sales_order: salesOrder, stage },
                freeze: true,
                silent: true,
    ...
    function showStageMoveErrorPopup(page, details) {
        ...
        modal.find("[data-stage-error-refresh]").on("click", function () {
            closeStageMoveErrorPopup();
            load(page, { boardScrollLeft: details.boardScrollLeft });
        });
    ...
    function stageMoveErrorMarkup(details, message, missingChecks) {
        ...
        const subtitle = __("{0} stays in its current column because the target status has required checks.", [details.record]);
        return `
                ...
                <section class="olp-stage-error-card">
                    ...
                    <div class="olp-stage-error-hero">
                        ...
                        <div>
                            ...
                            <h2 id="olp-stage-error-title">${frappe.utils.escape_html(__("Cannot move this {0}", [documentLabel]))}</h2>
    ...
    function openAssignDialog(page, cardName) {
        ...
        const dialog = new frappe.ui.Dialog({
            title: currentUser ? __("Change Assignment for {0}", [name]) : __("Assign Sales Order {0}", [name]),
            fields: [
            ...
            primary_action: async (values) => {
                ...
                await frappe.call({
                    ...
                    args: { document_type: "Sales Order", document_name: name, user },
                    freeze: true,
                });
                dialog.hide();
                frappe.show_alert({
                    message: user ? __("Sales Order {0} assigned to {1}", [name, user]) : __("Sales Order {0} unassigned", [name]),
                    indicator: "green",
                });
                load(page);
            },
    ...
    function cardMarkup(card) {
        ...
        const delivered = metricValue(card, "Delivered") || "0%";
        const deliveredPct = percentageNumber(delivered);
        const billed = metricValue(card, "Billed") || "0%";
        const amount = Number(card.amount || 0);
        const customer = card.title || "-";
        const workflowStatus = cleanStatusLabel(card.stage);
        if (isCollapsed) return collapsedCardMarkup(card, { erpStatus, delivered, billed, amount, customer, workflowStatus });
        return `
            <div class="olp-card ${isCollapsed ? "is-collapsed" : ""}" draggable="true" data-name="${frappe.utils.escape_html(card.name)}">
                    ...
                    <div class="olp-record-topbar">
                        ...
                        <div class="olp-card-tools">
                            ...
                            <div class="olp-stage-block">
                                ...
                                <div class="olp-dial ${progressTone(delivered)}" style="--pct:${deliveredPct};" title="${frappe.utils.escape_html(__("Delivered"))}: ${frappe.utils.escape_html(delivered)}">
                                    <span>${frappe.utils.escape_html(delivered)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="olp-signal status-${docTone(workflowStatus || erpStatus)}"><i></i>${frappe.utils.escape_html(salesOrderSignal(card, erpStatus, delivered, billed))}</div>
                    <h3>${frappe.utils.escape_html(customer)}</h3>
                ...
                ${isCollapsed ? "" : salesOrderActionsMarkup(card)}
    ...
    function createFromSalesOrder(action, salesOrderName) {
        if (!salesOrderName) return;
        const card = findCard(salesOrderName) || {};
        if (action === "delivery-note") {
            openMappedSalesOrderDoc(
    ...
    function openMappedSalesOrderDoc(method, doctype, salesOrderName, card) {
        if (frappe.model && frappe.model.open_mapped_doc) {
            frappe.model.open_mapped_doc({ method, source_name: salesOrderName });
            return;
        }
    ...
    function collapsedCardMarkup(card, context) {
        const status = context.workflowStatus || context.erpStatus || "-";
        return `
            <div class="olp-card is-collapsed" draggable="true" data-name="${frappe.utils.escape_html(card.name)}">
                <div class="olp-card-mini">
                    <div class="olp-mini-main">
                        <span class="olp-id-pill">${frappe.utils.escape_html(card.name)}</span>
                        <strong>${frappe.utils.escape_html(context.customer || card.title || card.name)}</strong>
                        <small>${frappe.utils.escape_html(card.subtitle || "-")}</small>
                    </div>
    ...
    function extraMetricsMarkup(card) {
        const reserved = new Set(["ERP Status", "Delivered"]);
    ...
    function salesOrderSignal(card, status, delivered, billed) {
        const workflowStatus = cleanStatusLabel(card.stage);
        if (workflowStatus) return workflowStatus;
        const clean = String(status || "").toLowerCase();
        if (["completed", "closed"].some((value) => clean.includes(value))) return __("Order completed");
        if (["cancelled"].some((value) => clean.includes(value))) return __("Order cancelled");
        if (percentageNumber(delivered) >= 100 && percentageNumber(billed) >= 100) return __("Delivered and billed");
        if (percentageNumber(delivered) > 0 || percentageNumber(billed) > 0) return __("Order in progress");
        return status || __("Status not set");
    }
    ...
    function percentageNumber(value) { return Math.max(0, Math.min(100, Number(String(value || "").replace("%", "")) || 0)); }
    function progressTone(value) { const percentage = percentageNumber(value); if (percentage >= 80) return "is-high"; if (percentage >= 35) return "is-medium"; return "is-low"; }
    function kpi(label, value) { return `<div class="olp-kpi"><span>${frappe.utils.escape_html(String(label || "-"))}</span><strong>${frappe.utils.escape_html(String(value == null ? "-" : value))}</strong></div>`; }
    function filterInput(id, label, value, placeholder, type = "text") { return `<label class="olp-filter-field" for="${frappe.utils.escape_html(id)}"><span>${frappe.utils.escape_html(label)}</span><input id="${frappe.utils.escape_html(id)}" type="${frappe.utils.escape_html(type)}" placeholder="${frappe.utils.escape_html(placeholder)}" value="${frappe.utils.escape_html(value)}" /></label>`; }
    function filterCombo(id, label, value, placeholder, options) {
        const cleanOptions = unique(options || []);
        return `<label class="olp-filter-field" for="${frappe.utils.escape_html(id)}"><span>${frappe.utils.escape_html(label)}</span><div class="olp-combo"><input id="${frappe.utils.escape_html(id)}" type="text" placeholder="${frappe.utils.escape_html(placeholder)}" value="${frappe.utils.escape_html(value)}" autocomplete="off" /><button type="button" class="olp-combo-toggle" data-combo-toggle="${frappe.utils.escape_html(id)}" aria-label="${frappe.utils.escape_html(__("Show options"))}">⌄</button><div class="olp-combo-menu"><button type="button" data-combo-option="">${frappe.utils.escape_html(placeholder)}</button>${cleanOptions.map((option) => `<button type="button" data-combo-option="${frappe.utils.escape_html(option)}">${frappe.utils.escape_html(option)}</button>`).join("")}<div class="olp-combo-empty">${frappe.utils.escape_html(__("No matching options"))}</div></div></div></label>`;
    }
    ...
    function injectPipelineStyles() {
        ...
        style.textContent = `
            ...
            .olp-filter-row--sales-primary { grid-template-columns: minmax(260px, 1.1fr) minmax(170px, .55fr) minmax(150px, .48fr) auto; }
            .olp-filter-row--sales-advanced { grid-template-columns: repeat(5, minmax(140px, 1fr)); margin-top: 9px; padding-top: 9px; border-top: 1px solid #eef2f7; }
            ...
            .olp-filter-row > button, .olp-filter-actions button { cursor: pointer; background: #0f172a; border-color: #0f172a; color: #fff; font-weight: 800; padding: 0 14px; white-space: nowrap; }
            .olp-filter-actions button[data-toggle-advanced], .olp-filter-actions button[data-reset-filters], .olp-filter-actions button[data-toggle-all-cards] { background: #f8fafc; border-color: #e2e8f0; color: #475569; }
            ...
            .olp-card { align-self: start; position: relative; overflow: hidden; border-radius: 18px; padding: 0; background: #fff; border: 1.5px solid #cbd5e1; box-shadow: 0 2px 5px rgba(15,23,42,.08), 0 18px 42px -24px rgba(15,23,42,.35); cursor: grab; transition: transform .22s cubic-bezier(.16, 1, .3, 1), box-shadow .22s cubic-bezier(.16, 1, .3, 1), border-color .18s ease, opacity .18s ease; }
            .olp-card::before { content: ""; position: absolute; left: 0; top: 0; bottom: 0; width: 4px; background: #0891b2; opacity: .72; z-index: 1; }
            ...
            .olp-stage-info strong { display: inline-flex; align-items: center; justify-content: flex-end; gap: 5px; max-width: 80px; color: #495061; font-size: 10px; font-weight: 800; line-height: 1.1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
            ...
            @media (max-width: 1500px) { .olp-filter-row--sales-primary, .olp-filter-row--sales-advanced { grid-template-columns: repeat(3, minmax(0, 1fr)); } .olp-filter-actions { grid-column: 1 / -1; justify-content: flex-start; } }
            @media (max-width: 1240px) { .olp-hero { flex-direction: column; gap: 12px; } .olp-hero-kpis { width: 100%; } .olp-filter-row--sales-primary, .olp-filter-row--sales-advanced { grid-template-columns: repeat(3, minmax(0, 1fr)); } .olp-filter-actions { align-items: stretch; } }
            ...
            @media (max-width: 760px) { .olp-hero, .olp-toolbar, .olp-stage-strip { padding-left: 14px; padding-right: 14px; } .olp-board { padding: 16px 14px 22px; scroll-padding-inline: 14px; } .olp-hero-kpis, .olp-filter-row--sales-primary, .olp-filter-row--sales-advanced { grid-template-columns: 1fr; } .olp-filter-actions { display: grid; grid-template-columns: repeat(2, 1fr); } .olp-column { flex-basis: 280px; } .olp-column.is-collapsed { flex-basis: 58px; } .olp-stage-error-modal { padding: 14px; } .olp-stage-error-hero { grid-template-columns: 1fr; padding-right: 54px; } .olp-stage-error-route { grid-template-columns: 1fr; } .olp-stage-error-route i { display: none; } .olp-stage-error-actions { display: grid; grid-template-columns: 1fr; } }
    ...
    function injectSalesOrderPipelineFilterStyles() {
        const style = document.getElementById("olp-sales-filter-style") || document.createElement("style");
        style.id = "olp-sales-filter-style";
        style.textContent = `
            .olp-sales-root .olp-filter-row--sales-primary { grid-template-columns: minmax(260px, 1.1fr) minmax(170px, .55fr) minmax(150px, .48fr) auto; }
            .olp-sales-root .olp-filter-row--sales-advanced { grid-template-columns: repeat(5, minmax(140px, 1fr)); margin-top: 9px; padding-top: 9px; border-top: 1px solid #eef2f7; }
            .olp-sales-root .olp-filter-actions { display: flex; flex-wrap: wrap; gap: 7px; align-items: end; min-width: 0; }
            .olp-sales-root .olp-filter-actions button { min-height: 36px; border: 1px solid #0f172a; background: #0f172a; color: #fff; border-radius: 12px; padding: 0 14px; font-size: 12px; font-weight: 800; cursor: pointer; white-space: nowrap; }
            .olp-sales-root .olp-filter-actions button[data-toggle-advanced], .olp-sales-root .olp-filter-actions button[data-reset-filters], .olp-sales-root .olp-filter-actions button[data-toggle-all-cards] { background: #f8fafc; border-color: #e2e8f0; color: #475569; }
            .olp-sales-root .olp-active-filters { display: flex; gap: 7px; flex-wrap: wrap; padding-top: 9px; }
            .olp-sales-root .olp-active-filters button { min-height: 30px; display: inline-flex; align-items: center; gap: 6px; border: 1px solid #bae6fd; border-radius: 999px; background: #ecfeff; color: #0e7490; padding: 0 10px; font-size: 11px; font-weight: 850; cursor: pointer; }
            .olp-sales-root .olp-active-filters button span { color: #64748b; font-size: 10px; text-transform: uppercase; letter-spacing: .06em; }
            @media (max-width: 1500px) { .olp-sales-root .olp-filter-row--sales-primary, .olp-sales-root .olp-filter-row--sales-advanced { grid-template-columns: repeat(3, minmax(0, 1fr)); } .olp-sales-root .olp-filter-actions { grid-column: 1 / -1; justify-content: flex-start; } }
            @media (max-width: 1240px) { .olp-sales-root .olp-filter-row--sales-primary, .olp-sales-root .olp-filter-row--sales-advanced { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
            @media (max-width: 760px) { .olp-sales-root .olp-filter-row--sales-primary, .olp-sales-root .olp-filter-row--sales-advanced { grid-template-columns: 1fr; } .olp-sales-root .olp-filter-actions { display: grid; grid-template-columns: repeat(2, 1fr); } }
        `;
        document.head.appendChild(style);
    }
})();
