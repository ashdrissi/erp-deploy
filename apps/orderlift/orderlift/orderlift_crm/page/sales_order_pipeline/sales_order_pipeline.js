(function () {
        ...
        page.main.html(`
            <div class="olp-shell">
                ...
                <section class="olp-toolbar">
                    <div class="olp-filter-row olp-filter-row--sales">
    ...
    function bind(page) {
        ...
        page.main.find(".olp-column[data-column!='__unassigned__']").on("drop", async function (event) {
            ...
            try {
                await frappe.call({ method: "orderlift.orderlift_crm.api.pipeline.update_sales_order_stage", args: { sales_order: name, stage }, freeze: true });
                frappe.show_alert({ message: __("Sales Order {0} moved to {1}", [name, stage]), indicator: "green" });
                load(page);
            } finally {
                STATE.dragged = null;
            }
    ...
    function injectPipelineStyles() {
        ...
        style.textContent = `
            ...
            .olp-filter-row--sales { grid-template-columns: minmax(240px, 1.2fr) repeat(7, minmax(120px, .45fr)) auto; }
            ...
            @media (max-width: 1240px) { .olp-hero { flex-direction: column; gap: 12px; } .olp-hero-kpis { width: 100%; } .olp-filter-row--sales { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
            ...
            @media (max-width: 760px) { .olp-hero, .olp-toolbar, .olp-stage-strip { padding-left: 14px; padding-right: 14px; } .olp-board { padding: 16px 14px 22px; scroll-padding-inline: 14px; } .olp-hero-kpis, .olp-filter-row--sales { grid-template-columns: 1fr; } .olp-column { flex-basis: 280px; } .olp-column.is-collapsed { flex-basis: 58px; } }
        `;
