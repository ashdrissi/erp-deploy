(function () {
    ...
    frappe.pages["project-pipeline"].on_page_load = function (wrapper) {
        ...
        wrapper.page = page;
        page.main.addClass("olp-root olp-project-root");
        injectPipelineStyles();
        injectProjectPipelineFilterStyles();
        render(page);
        load(page);
        header(page);
    };
    ...
    frappe.pages["project-pipeline"].on_page_show = function (wrapper) {
        ...
        wrapper.page.main.addClass("olp-root olp-project-root");
        injectPipelineStyles();
        injectProjectPipelineFilterStyles();
        header(wrapper.page);
        load(wrapper.page);
                ...
                <section class="olp-toolbar">
                    <div class="olp-filter-row olp-filter-row--project">
                        ...
                        <div class="olp-filter-actions">
                            <button data-create-project="1">${__("Create Project")}</button>
    ...
    function bind(page) {
        ...
        page.main.find("[data-create-project]").on("click", createProject);
        ...
        page.main.find("[data-create-from-project]").on("click", function (event) {
            event.preventDefault();
            event.stopPropagation();
            createFromProject($(this).attr("data-create-from-project"), $(this).attr("data-card"));
        });
        page.main.find(".olp-card").on("click", function (event) {
            if ($(event.target).closest("[data-toggle-card], [data-assign-card], [data-create-from-project]").length) return;
        ...
        page.main.find(".olp-column[data-column!='__unassigned__']").on("drop", async function (event) {
            ...
            if (targetColumn && targetColumn.confirmation_message) {
                showStageMoveConfirmationPopup(page, {
                    documentLabel: __("Project"),
                    record: name,
                    stage,
                    message: targetColumn.confirmation_message,
                    boardScrollLeft,
                    onConfirm: () => moveProjectCard(page, name, stage, boardScrollLeft),
                });
                STATE.dragged = null;
                return;
            }
            await moveProjectCard(page, name, stage, boardScrollLeft);
            STATE.dragged = null;
        });
    ...
    async function moveProjectCard(page, name, stage, boardScrollLeft) {
        try {
            ...
            const message = extractServerMessage(error) || __("This project cannot move to {0} yet.", [stage]);
            showStageMoveErrorPopup(page, { documentLabel: __("Project"), record: name, stage, message, boardScrollLeft });
        } finally {
            STATE.dragged = null;
        }
    }
    ...
    function clearFilter(key) {
        const defaults = { search: "", company: "All", owner: "All", status: "All", business_type: "All", segment: "All" };
    ...
    function updateProjectStage(project, stage) {
        return new Promise((resolve, reject) => {
            frappe.call({
                method: "orderlift.orderlift_crm.api.pipeline.update_project_stage",
                args: { project, stage },
                freeze: true,
                silent: true,
    ...
    function openAssignDialog(page, cardName) {
        ...
        const dialog = new frappe.ui.Dialog({
            title: currentUser ? __("Change Assignment for {0}", [name]) : __("Assign Project {0}", [name]),
            fields: [
            ...
            primary_action: async (values) => {
                ...
                await frappe.call({
                    ...
                    args: { document_type: "Project", document_name: name, user },
                    freeze: true,
                });
                dialog.hide();
                frappe.show_alert({
                    message: user ? __("Project {0} assigned to {1}", [name, user]) : __("Project {0} unassigned", [name]),
                    indicator: "green",
                });
                load(page);
            },
    ...
    function cardMarkup(card) {
        ...
        if (isCollapsed) return collapsedCardMarkup(card, { erpStatus, customer, docCount, workflowStatus });
        return `
            <div class="olp-card ${isCollapsed ? "is-collapsed" : ""}" draggable="true" data-name="${frappe.utils.escape_html(card.name)}">
                ...
                ${isCollapsed ? "" : projectActionsMarkup(card)}
    ...
    function createFromProject(action, projectName) {
        if (!projectName) return;
        const card = findCard(projectName) || {};
            ...
            <div class="olp-card is-collapsed" draggable="true" data-name="${frappe.utils.escape_html(card.name)}">
                <div class="olp-card-mini">
                    <div class="olp-mini-main">
                        ...
                        <small>${frappe.utils.escape_html(card.subtitle || context.customer || "-")}</small>
                    </div>
                    <div class="olp-mini-facts">
                        <span class="status-${docTone(status)}">${frappe.utils.escape_html(status)}</span>
                        <span>${context.docCount} ${__("docs")}</span>
                    </div>
    ...
    function assignmentPill(card) {
        ...
        if (!frappe.boot?.orderlift_capabilities?.project_pipeline_assignment) {
    ...
    function injectPipelineStyles() {
        ...
        style.textContent = `
            ...
            @media (max-width: 1500px) { .olp-filter-row--project { grid-template-columns: repeat(3, minmax(0, 1fr)); } .olp-filter-actions { grid-column: 1 / -1; justify-content: flex-start; } }
            @media (max-width: 1240px) { .olp-hero { flex-direction: column; gap: 12px; } .olp-hero-kpis { width: 100%; } .olp-filter-row--project { grid-template-columns: repeat(3, minmax(0, 1fr)); } .olp-filter-actions { align-items: stretch; } }
            ...
            @media (max-width: 760px) { .olp-hero, .olp-toolbar, .olp-stage-strip { padding-left: 14px; padding-right: 14px; } .olp-board { padding: 16px 14px 22px; scroll-padding-inline: 14px; } .olp-hero-kpis, .olp-filter-row--project { grid-template-columns: 1fr; } .olp-filter-actions { display: grid; grid-template-columns: repeat(3, 1fr); } .olp-column { flex-basis: 280px; } .olp-column.is-collapsed { flex-basis: 58px; } .olp-stage-error-modal { padding: 14px; } .olp-stage-error-hero { grid-template-columns: 1fr; padding-right: 54px; } .olp-stage-error-route { grid-template-columns: 1fr; } .olp-stage-error-route i { display: none; } .olp-stage-error-actions { display: grid; grid-template-columns: 1fr; } }
    ...
    function injectProjectPipelineFilterStyles() {
        const style = document.getElementById("olp-project-filter-style") || document.createElement("style");
        style.id = "olp-project-filter-style";
        style.textContent = `
            .olp-project-root .olp-filter-actions { display: flex; flex-wrap: wrap; gap: 7px; align-items: end; min-width: 0; }
            .olp-project-root .olp-filter-actions button { min-height: 36px; border: 1px solid #0f172a; background: #0f172a; color: #fff; border-radius: 12px; padding: 0 14px; font-size: 12px; font-weight: 800; cursor: pointer; white-space: nowrap; }
            .olp-project-root .olp-filter-actions button[data-reset-filters], .olp-project-root .olp-filter-actions button[data-toggle-all-cards] { background: #f8fafc; border-color: #e2e8f0; color: #475569; }
            .olp-project-root .olp-active-filters { display: flex; gap: 7px; flex-wrap: wrap; padding-top: 9px; }
            .olp-project-root .olp-active-filters button { min-height: 30px; display: inline-flex; align-items: center; gap: 6px; border: 1px solid #bae6fd; border-radius: 999px; background: #ecfeff; color: #0e7490; padding: 0 10px; font-size: 11px; font-weight: 850; cursor: pointer; }
            .olp-project-root .olp-active-filters button span { color: #64748b; font-size: 10px; text-transform: uppercase; letter-spacing: .06em; }
            @media (max-width: 1500px) { .olp-project-root .olp-filter-row--project { grid-template-columns: repeat(3, minmax(0, 1fr)); } .olp-project-root .olp-filter-actions { grid-column: 1 / -1; justify-content: flex-start; } }
            @media (max-width: 1240px) { .olp-project-root .olp-filter-row--project { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
            @media (max-width: 760px) { .olp-project-root .olp-filter-row--project { grid-template-columns: 1fr; } .olp-project-root .olp-filter-actions { display: grid; grid-template-columns: repeat(2, 1fr); } }
        `;
        document.head.appendChild(style);
    }
})();
