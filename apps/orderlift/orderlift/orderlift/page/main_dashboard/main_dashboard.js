...
class MainDashboard {
	...
	refresh() {
		frappe.call({
			...
			error: () => {
				// On error still show gateways — just leave KPI cards empty
				this.wrapper.find('#kpi-container').html('');
			}
		});
	}

	render_kpis(stats) {
		const curr = frappe.boot.sysdefaults.currency || 'MAD';
		const fmt_curr = (val) => {
			return frappe.format(val, { fieldtype: 'Currency', options: curr }, { inline: true });
		};
		const kpis = [
			{ label: "Sales (MTD)", value: fmt_curr(stats.sales_mtd), color: "var(--blue-500)", bg: "var(--blue-50)", icon: `<path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>` },
			{ label: "Pending Quotes", value: stats.quotes_pending, color: "var(--orange-500)", bg: "var(--orange-50)", icon: `<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line>` },
