...
class TestIntercompanyAutomation(unittest.TestCase):
    def test_purchase_order_after_insert_creates_draft_sales_order(self):
        hooks = (APP_ROOT / "hooks.py").read_text()
        source = (APP_ROOT / "intercompany.py").read_text()

        self.assertIn('"after_insert": "orderlift.intercompany.create_draft_sales_order_from_purchase_order"', hooks)
        for token in [
