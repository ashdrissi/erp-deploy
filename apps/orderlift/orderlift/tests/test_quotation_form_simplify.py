...
class TestQuotationFormSimplify(unittest.TestCase):
    ...
    def test_direct_quotation_discount_editing_is_wired(self):
        ...
        self.assertIn("async custom_pu_ttc(frm, cdt, cdn)", script)
        self.assertIn("await applyTTCPriceFromOverride(frm, row)", script)
        self.assertIn("async function applyResolvedNetRate", script)
