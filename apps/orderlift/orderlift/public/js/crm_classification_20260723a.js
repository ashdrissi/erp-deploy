(function () {
    ...
    async function showOpportunityPreFormDialog(defaults = {}) {
        ...
        const dialog = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                { fieldname: "deal_abbreviation", label: __("Deal Abbreviation"), fieldtype: "Data", description: __("Optional: 2-12 letters or numbers used across related documents.") },
    ...
    frappe.ui.form.on("Quotation", {
        refresh(frm) {
            ...
            // Party classification/defaults sync intentionally run only on
            // quotation_to / party_name changes (handlers below), not here.
            // refresh fires after every save and re-running these async
            // fetches re-dirties the form, causing the save loop.
    ...
    async function syncTransactionPartyDefaults(frm, mode, applyValues) {
        ...
        try {
            ...
            const values = {
                ...
                contact_email: data.email || "",
                shipping_address_name: data.shipping_address_name || data.billing_address_name || data.address_name || "",
                shipping_address: data.shipping_address_display || data.billing_address_display || data.address || "",
            };
            for (const [fieldname, value] of Object.entries(values)) {
                await setAutoValue(frm, fieldname, value, applyValues);
            }
    ...
    function renderPartyClassificationBar(frm) {
        ...
        if (frm.$wrapper) frm.$wrapper.find(".ol-crm-classification-bar").remove();
        ...
        insertClassificationBar(frm, `<div class="ol-crm-classification-bar ol-order-status-flow" role="list">${chips.join("")}</div>`);
    }

    function renderOpportunityStatusBar(frm) {
        injectCrmClassificationStyles();
        frm.page.inner_toolbar.find(".ol-opportunity-status-bar").remove();
        if (frm.$wrapper) frm.$wrapper.find(".ol-opportunity-status-bar").remove();
        const chips = [
        ...
        insertClassificationBar(frm, `<div class="ol-opportunity-status-bar ol-order-status-flow" role="list">${chips.join("")}</div>`);
    }
    ...
    function renderTransactionClassificationBar(frm, label) {
        ...
        if (frm.$wrapper) frm.$wrapper.find(".ol-transaction-classification-bar").remove();
        const chips = [
            crmChip(__("Type"), frm.doc.custom_crm_business_type || __("Not set"), "type"),
            crmChip(__("Segment"), frm.doc.custom_crm_segment || __("Not set"), "segment"),
        ];
        insertClassificationBar(frm, `
            <div class="ol-transaction-classification-bar ol-order-status-flow" role="list" title="${frappe.utils.escape_html(label)} CRM Classification">
                ${chips.join("")}
            </div>
        `);
    }

    function insertClassificationBar(frm, html) {
        const bar = $(html);
    ...
    function crmChip(label, value, tone) {
        ...
        return `
            <span class="ol-order-status-step ol-status-${color}" data-tone="${frappe.utils.escape_html(tone || "")}" role="listitem" title="${title}" aria-label="${title}">
                <span class="ol-status-step-icon">${crmIcon(tone)}</span>
                <span class="ol-status-step-text">${frappe.utils.escape_html(displayValue)}</span>
            </span>
        `;
    }

    function crmIcon(tone) {
        const icon = tone === "status" ? "check" : tone === "type" ? "truck" : tone === "segment" ? "tool" : "tag";
    ...
    function injectCrmClassificationStyles() {
        ...
        style.textContent = `
            .ol-crm-classification-bar.ol-order-status-flow, .ol-opportunity-status-bar.ol-order-status-flow, .ol-transaction-classification-bar.ol-order-status-flow { display:flex; align-items:center; gap:5px; flex-wrap:wrap; padding:6px; margin:0 0 6px; border:1px solid var(--border-color, #d8dee8); border-radius:9px; background:linear-gradient(135deg, rgba(255,255,255,.96), rgba(248,250,252,.9)); }
            .ol-crm-classification-bar .ol-order-status-step, .ol-opportunity-status-bar .ol-order-status-step, .ol-transaction-classification-bar .ol-order-status-step { position:relative; display:inline-flex; align-items:center; gap:5px; min-height:24px; min-width:0; max-width:180px; padding:3px 8px; border-radius:999px; font-size:11px; line-height:1.2; font-weight:600; letter-spacing:0; text-decoration:none; white-space:nowrap; }
            .ol-crm-classification-bar .ol-order-status-step:not(:last-child)::after, .ol-opportunity-status-bar .ol-order-status-step:not(:last-child)::after, .ol-transaction-classification-bar .ol-order-status-step:not(:last-child)::after { content:""; position:absolute; left:calc(100% + 4px); top:50%; width:8px; height:2px; transform:translateY(-50%); border-radius:999px; background:currentColor; opacity:.38; }
            .ol-crm-classification-bar .ol-status-step-icon, .ol-opportunity-status-bar .ol-status-step-icon, .ol-transaction-classification-bar .ol-status-step-icon { display:inline-flex; align-items:center; justify-content:center; flex:0 0 auto; }
            .ol-crm-classification-bar .ol-status-step-icon .icon, .ol-opportunity-status-bar .ol-status-step-icon .icon, .ol-transaction-classification-bar .ol-status-step-icon .icon { stroke:currentColor; }
            .ol-opportunity-status-bar .ol-order-status-step[data-tone="status"] { max-width:180px; }
            .ol-crm-classification-bar .ol-status-step-text, .ol-opportunity-status-bar .ol-status-step-text, .ol-transaction-classification-bar .ol-status-step-text { display:block; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
            .ol-crm-classification-bar .ol-status-blue, .ol-opportunity-status-bar .ol-status-blue, .ol-transaction-classification-bar .ol-status-blue { color:#0b5ed7; background:linear-gradient(135deg, #eef5ff, #f7fbff); }
            .ol-crm-classification-bar .ol-status-green, .ol-opportunity-status-bar .ol-status-green, .ol-transaction-classification-bar .ol-status-green { color:#0f8f72; background:linear-gradient(135deg, #e9f8f3, #f5fbf8); }
            .ol-crm-classification-bar .ol-status-orange, .ol-opportunity-status-bar .ol-status-orange, .ol-transaction-classification-bar .ol-status-orange { color:#d96b0b; background:linear-gradient(135deg, #fff3e8, #fff8f1); }
            .ol-crm-classification-bar .ol-status-purple, .ol-opportunity-status-bar .ol-status-purple, .ol-transaction-classification-bar .ol-status-purple { color:#7c3aed; background:linear-gradient(135deg, #f4efff, #fbf8ff); }
            .ol-crm-classification-bar .ol-status-red, .ol-opportunity-status-bar .ol-status-red, .ol-transaction-classification-bar .ol-status-red { color:#d92d20; background:linear-gradient(135deg, #fff1f0, #fff8f7); }
            .ol-crm-classification-bar .ol-status-gray, .ol-opportunity-status-bar .ol-status-gray, .ol-transaction-classification-bar .ol-status-gray { color:#475569; background:linear-gradient(135deg, #f1f5f9, #f8fafc); }
            @media (max-width: 767px) {
                .ol-crm-classification-bar.ol-order-status-flow, .ol-opportunity-status-bar.ol-order-status-flow, .ol-transaction-classification-bar.ol-order-status-flow { gap:4px; margin-bottom:5px; }
                .ol-crm-classification-bar .ol-order-status-step, .ol-opportunity-status-bar .ol-order-status-step, .ol-transaction-classification-bar .ol-order-status-step { flex:1 1 132px; justify-content:center; max-width:none; padding:3px 7px; font-size:10px; }
                .ol-crm-classification-bar .ol-order-status-step:not(:last-child)::after, .ol-opportunity-status-bar .ol-order-status-step:not(:last-child)::after, .ol-transaction-classification-bar .ol-order-status-step:not(:last-child)::after { display:none; }
            }
