 ...
 * except for the section containing the active item.
 */

(function normalizeMainDashboardSectionState() {
    if (window.__orderlift_main_dashboard_section_state_20260423g_installed) return;
    window.__orderlift_main_dashboard_section_state_20260423g_installed = true;
    ...
    function hasActiveItem(sectionItem) {
        return !!(
            sectionItem &&
            sectionItem.wrapper &&
            sectionItem.wrapper.find &&
            sectionItem.wrapper.find(".active-sidebar").length
        );
    }
    ...
    function collapseSectionsOnce() {
        ...
        for (var i = 0; i < sidebar.items.length; i++) {
            ...
            var keepOpen = hasActiveItem(item);

            if (keepOpen && typeof item.open === "function") {
                item.open();
                collapsedLabels[item.item.label] = false;
                continue;
            }
