...
function renderPriceApprovalSection(frm, access) {
    ...
    if (!rows.length) {
        return `<section class="ol-po-price-approval success"><strong>${__("Pricing Alerts & Approvals")}</strong><div>${__("No negotiated prices require approval.")}</div></section>`;
    }
    ...
    const rowsHtml = rows.map((row) => {
        ...
        const action = status === "Pending" && canApprove && !frm.doc.__islocal
            ? `<button class="btn btn-xs btn-primary ol-po-price-review" data-decision="Approved" data-item="${escapeHtml(row.name)}">${__("Approve & Update Price List")}</button><button class="btn btn-xs btn-default ol-po-price-review" data-decision="Skipped" data-item="${escapeHtml(row.name)}">${__("Skip Update")}</button>`
            : "";
        return `<tr><td>${escapeHtml(row.item_code || "")}</td><td>${escapeHtml(row.custom_source_buying_price_list || "")}</td><td>${formatCurrency(frm, row.custom_loaded_buying_rate)}</td><td>${formatCurrency(frm, row.rate)}</td><td>${formatCurrency(frm, row.custom_price_variance_amount)}</td><td><span class="ol-po-alert-pill ${status === "Pending" ? "warning" : "success"}">${__(status)}</span></td><td>${action}</td></tr>`;
    }).join("");
    const note = frm.doc.__islocal
        ? __("Save this Purchase Order before reviewing negotiated prices.")
        : canApprove
            ? __("Approve each negotiated price only after attesting that the approved rate may update the source buying price list.")
            : __("Pending privileged pricing approval. You can see the alert but cannot approve or update the buying price list.");
    return `<section class="ol-po-price-approval ${pending.length ? "warning" : "success"}"><div class="ol-po-price-approval-header"><strong>${__("Pricing Alerts & Approvals")}</strong><span>${pending.length} ${__("pending")}</span></div><p>${note}</p><div class="table-responsive"><table class="table table-bordered table-condensed"><thead><tr><th>${__("Item")}</th><th>${__("Price List")}</th><th>${__("Loaded")}</th><th>${__("PO Price")}</th><th>${__("Difference")}</th><th>${__("Status")}</th><th>${__("Action")}</th></tr></thead><tbody>${rowsHtml}</tbody></table></div></section>`;
}
...
function bindPriceReviewActions(frm) {
    ...
    wrapper.on("click.orderliftPriceReview", ".ol-po-price-review", function () {
        ...
        if (decision === "Approved") {
            frappe.confirm(
                __("I confirm this approved negotiated price may update the source buying price list on Purchase Order submission."),
                () => submitPriceReview(frm, itemName, decision, 1),
            );
        } else {
...
async function submitPriceReview(frm, itemName, decision, attestation) {
    try {
        await frappe.call({
            method: "orderlift.orderlift_sales.utils.purchase_order_pricing.set_purchase_order_price_review_decisions",
            args: {
                decisions: JSON.stringify([{ purchase_order: frm.doc.name, purchase_order_item: itemName, decision }]),
                attestation,
            },
        });
...
function ensurePricingAlertStyles() {
    ...
    style.textContent = `
        ...
        .ol-po-price-approval table { margin-bottom: 0; background: #fff; }
        .ol-po-price-approval .btn { margin: 2px; }
    `;
