...
(function orderliftSidebarTunePatch() {
    ...
    function readSectionBreakState() {
        try {
    ...
    function writeSectionBreakState(state) {
        try {
    ...
    function collapseMainDashboardSections() {
        var sidebar = getSidebar();
        if (!sidebar || !Array.isArray(sidebar.items) || getSidebarKey() !== MAIN_DASHBOARD_KEY) return;
        ...
        for (var i = 0; i < sidebar.items.length; i++) {
            ...
            var hasActiveChild = !!(item.wrapper && item.wrapper.find(".active-sidebar").length);
            nextState[item.item.label] = !hasActiveChild;

            if (hasActiveChild) {
                if (typeof item.open === "function") item.open();
            } else if (typeof item.close === "function") {
                item.close();
            }
        }

        var storedState = readSectionBreakState();
        storedState[MAIN_DASHBOARD_KEY] = nextState;
        writeSectionBreakState(storedState);
    }
