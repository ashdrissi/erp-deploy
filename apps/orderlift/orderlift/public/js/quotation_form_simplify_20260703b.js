(function () {
    ...
    function addQuotationPrintShortcut(frm) {
        if (!frm || !frm.page || !frm.page.wrapper || frm.is_new()) return;

        const page = $(frm.page.wrapper);
        page.find(".ol-print-shortcut-group, .ol-print-shortcut").remove();

        const actions = page.find(".page-actions").first();
        if (!actions.length) return;

        ensureQuotationPrintShortcutStyles();
        const iconPDF = frappe.utils && frappe.utils.icon ? frappe.utils.icon("file", "sm") : "";
        const iconPrint = frappe.utils && frappe.utils.icon ? frappe.utils.icon("printer", "sm") : "";
        const btnPDF = $(`
            <button type="button" class="btn btn-default btn-sm ol-print-shortcut" title="${__("PDF")}" aria-label="${__("PDF")}">
                ${iconPDF}
                <span class="hidden-xs">${frappe.utils.escape_html(__("PDF"))}</span>
            </button>
        `);
        btnPDF.on("click", () => {
            const format = (frm.print_preview && frm.print_preview.print_format) || "Standard";
            const no_letterhead = (frm.print_preview && frm.print_preview.no_letterhead != null) ? frm.print_preview.no_letterhead : 0;
            const url = `/api/method/frappe.utils.print_format.download_pdf?doctype=${encodeURIComponent(frm.doctype)}&name=${encodeURIComponent(frm.doc.name)}&format=${encodeURIComponent(format)}&no_letterhead=${no_letterhead}`;
            window.open(url, "_blank");
        });
        const btnPrint = $(`
            <button type="button" class="btn btn-default btn-sm ol-print-shortcut" title="${__("Print")}" aria-label="${__("Print")}">
                ${iconPrint}
                <span class="hidden-xs">${frappe.utils.escape_html(__("Print"))}</span>
            </button>
        `);
        btnPrint.on("click", () => {
        ...
        const wrapper = $(`<span class="ol-print-shortcut-group"></span>`);
        wrapper.prepend(btnPrint);
        wrapper.prepend(btnPDF);
        actions.prepend(wrapper);
    }

    function ensureQuotationPrintShortcutStyles() {
        if (document.getElementById("ol-print-shortcut-style")) return;
        $("head").append(`
            <style id="ol-print-shortcut-style">
                .ol-print-shortcut {
                    display: inline-flex;
                    align-items: center;
                    gap: 7px;
