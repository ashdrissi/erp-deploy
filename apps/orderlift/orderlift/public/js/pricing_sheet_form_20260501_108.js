...
function renderPricingSheetWorkspace(frm) {
    ...
    $root.find("[data-ps-order-item]").on("dragstart", (event) => {
        ...
        if (nativeEvent && nativeEvent.dataTransfer) {
            nativeEvent.dataTransfer.setData("text/plain", draggedFieldname || "");
    ...
    $root.find("[data-ps-order-item]").on("drop", (event) => {
        event.preventDefault();
        if (!draggedFieldname && event.originalEvent && event.originalEvent.dataTransfer) {
            draggedFieldname = event.originalEvent.dataTransfer.getData("text/plain");
        }
