(function () {
    ...
    function applyQuotationItemPricingLayout(frm) {
        ...
        if (grid.get_field && grid.get_field("source_price_list_sell_rate")) {
            ...
            grid.update_docfield_property("source_price_list_sell_rate", "precision", "2");
        }
        if (grid.get_field && grid.get_field("source_gross_sell_rate")) {
            grid.update_docfield_property("source_gross_sell_rate", "label", __("PU HT"));
            grid.update_docfield_property("source_gross_sell_rate", "read_only", 0);
            grid.update_docfield_property("source_gross_sell_rate", "hidden", 0);
            grid.update_docfield_property("source_gross_sell_rate", "in_list_view", 1);
            grid.update_docfield_property("source_gross_sell_rate", "precision", "2");
        }
        if (grid.get_field && grid.get_field("source_discounted_sell_rate")) {
            grid.update_docfield_property("source_discounted_sell_rate", "label", __("PU HT net"));
        ...
        if (grid.get_field && grid.get_field("amount")) {
            grid.update_docfield_property("amount", "label", __("PT HT net"));
        ...
        if (grid.get_field && grid.get_field("source_discount_amount")) {
            grid.update_docfield_property("source_discount_amount", "label", __("Remise HT"));
        ...
        if (grid.get_field && grid.get_field("custom_pu_ttc")) {
            grid.update_docfield_property("custom_pu_ttc", "label", __("PU TTC net"));
        ...
        if (grid.get_field && grid.get_field("custom_pt_ttc")) {
            grid.update_docfield_property("custom_pt_ttc", "label", __("PT TTC net"));
    ...
    function enforceQuotationItemGridColumns(frm) {
        ...
        (grid.docfields || []).forEach((df) => {
            ...
            if (column) {
                ...
                if (["source_discounted_sell_rate", "amount", "custom_pt_ttc"].includes(df.fieldname)) df.read_only = 1;
            } else {
                df.in_list_view = 0;
                df.columns = 0;
                df.sticky = 0;
    ...
    function ensureQuotationItemsGridStyles() {
        const css = `
                ...
                .orderlift-inline-items-grid .grid-row,
                .orderlift-inline-items-grid .grid-row .data-row,
                .orderlift-inline-items-grid .grid-editable-row {
                ...
                .orderlift-inline-items-grid .grid-editable-row {
                    min-width: ${canViewQuotationMargins() ? "2060px" : "1780px"} !important;
                }
                .orderlift-inline-items-grid .row-check,
                .orderlift-inline-items-grid .grid-row-check,
                .orderlift-inline-items-grid .grid-static-col.row-check,
                .orderlift-inline-items-grid .grid-heading-row .row-check,
                .orderlift-inline-items-grid .data-row .row-check,
                .orderlift-inline-items-grid .grid-heading-row .grid-row-check,
                .orderlift-inline-items-grid .data-row .grid-row-check {
                ...
                .orderlift-inline-items-grid .grid-static-col:has(.btn-open-row),
                .orderlift-inline-items-grid .col:has(.btn-open-row) {
        ...
        `;
        const existing = document.getElementById("orderlift-quotation-items-grid-style");
        if (existing) {
            existing.textContent = css;
            return;
        }
        $("head").append(`<style id="orderlift-quotation-items-grid-style">${css}</style>`);
    }
    ...
    async function addOtherChargeRow(frm, values, dialog, item) {
        ...
        const amount = roundCurrency(qty * unitAmount);
    ...
    function syncGrossRateIfNeeded(row) {
        ...
        const fallbackRate = Number(row.price_list_rate || row.rate || 0);
        if (("source_price_list_sell_rate" in row) && !Number(row.source_price_list_sell_rate || 0)) {
            frappe.model.set_value(row.doctype, row.name, "source_price_list_sell_rate", fallbackRate);
        }
        if (Number(row.source_gross_sell_rate || 0)) return;
        frappe.model.set_value(row.doctype, row.name, "source_gross_sell_rate", fallbackRate);
    }
