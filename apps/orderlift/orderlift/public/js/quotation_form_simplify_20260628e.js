(function () {
    ...
    frappe.ui.form.on("Quotation Item", {
        ...
        source_discounted_sell_rate(frm, cdt, cdn) {
            if (!canOverrideQuotationPricing()) return;
    ...
    function applyQuotationItemPricingLayout(frm) {
        ...
        if (grid.get_field && grid.get_field("source_discounted_sell_rate")) {
            ...
            grid.update_docfield_property("source_discounted_sell_rate", "read_only", isAdmin ? 0 : 1);
        ...
        if (grid.get_field && grid.get_field("source_discount_percent")) {
            grid.update_docfield_property("source_discount_percent", "label", __("Pricing Discount %"));
    ...
    function applyPricingDiscount(frm, row, options = {}) {
        ...
        if (!gross) return;
        const isAdmin = canOverrideQuotationPricing();
        const maxDiscount = isAdmin ? Infinity : Number(row.source_max_discount_percent || 0);
