(function (global) {
    ...
    function init(root, options) {
        ...
        // Leaflet injects several anchor tags into the map surface. Prevent
        // in-map utility controls from triggering browser navigation/flicker.
        root.addEventListener("click", (event) => {
            const anchor = event.target.closest("a");
            if (!anchor) return;

            const href = anchor.getAttribute("href") || "";
            const isLeafletControl = anchor.closest(".leaflet-control-container");
            const isLeafletAttribution = anchor.closest(".leaflet-control-attribution");

            if (href === "#" || (isLeafletControl && !isLeafletAttribution)) {
                event.preventDefault();
            }
        });
        ...
        function renderMarkers(projects) {
            ...
            lastBounds = bounds;
            if (bounds.length) {
                refreshMapLayout(bounds, { refit: true });
            } else {
                refreshMapLayout();
            }
        }
        ...
        function refreshMapLayout(bounds, options) {
            if (!map) return;

            const refit = Boolean(options && options.refit && bounds && bounds.length);
