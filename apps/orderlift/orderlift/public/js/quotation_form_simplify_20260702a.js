(function () {
    ...
    const QUOTATION_ITEM_GRID_COLUMNS = [
        { fieldname: "item_code", columns: 4, sticky: 0 },
        { fieldname: "qty", columns: 1, sticky: 0 },
        { fieldname: "source_discounted_sell_rate", columns: 2, sticky: 0 },
        { fieldname: "amount", columns: 2, sticky: 0 },
        { fieldname: "source_max_discount_percent", columns: 2, sticky: 0 },
        { fieldname: "source_discount_percent", columns: 2, sticky: 0 },
        { fieldname: "source_discount_amount", columns: 2, sticky: 0 },
        { fieldname: "custom_pu_ttc", columns: 2, sticky: 0 },
        { fieldname: "custom_pt_ttc", columns: 2, sticky: 0 },
    ];
    ...
    function addQuotationPrintShortcut(frm) {
        ...
        const icon = frappe.utils && frappe.utils.icon ? frappe.utils.icon("printer", "sm") : "";
        const button = $(`
    ...
    function enforceQuotationItemGridColumns(frm) {
        ...
        (grid.docfields || []).forEach((df) => {
            ...
            } else if (!df.reqd && !["item_code", "qty"].includes(df.fieldname)) {
                df.in_list_view = 0;
            }
        });
    ...
    function applyTTCPriceFromOverride(frm, row) {
        if (!frm || !row) return;
        const targetTTC = Number(row.custom_pu_ttc || 0);
        if (!targetTTC || targetTTC < 0) return;
        applyResolvedNetRate(frm, row, netRateFromTTC(targetTTC, quotationTotalTaxRate(frm)));
    }

    function applyResolvedNetRate(frm, row, requestedNetRate, options = {}) {
        if (!frm || !row) return;
        const gross = Number(row.source_gross_sell_rate || row.price_list_rate || 0);
        if (!gross) return;
        const isAdmin = canOverrideQuotationPricing();
        const configuredMaxDiscount = Number(row.source_max_discount_percent || 0);
        const maxDiscount = isAdmin ? Infinity : configuredMaxDiscount;
        ...
        try {
            ...
            if ("source_discount_amount" in row) frappe.model.set_value(row.doctype, row.name, "source_discount_amount", Math.max(gross - netRate, 0));
    ...
    function syncItemTTCFields(frm) {
        ...
        (frm.doc.items || []).forEach(function (row) {
            ...
            var amount = rate * qty;
            var taxAmount = roundCurrency(amount * totalTaxRate / 100);
            ...
            setItemFieldIfChanged(row, "custom_applied_taxes", taxAmount);
            setItemFieldIfChanged(row, "custom_pu_ttc", puTtc);
            setItemFieldIfChanged(row, "custom_pt_ttc", ptTtc);
        });
    }
