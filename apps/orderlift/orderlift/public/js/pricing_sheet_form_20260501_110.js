...
function getPricingSheetWorkspaceStorageKey() {
    const user = (frappe.session && frappe.session.user) || "guest";
    return `${PRICING_SHEET_WORKSPACE_STORAGE_KEY}.${user}`;
}
...
function savePricingSheetWorkspaceColumns(fieldnames) {
    try {
        window.localStorage.setItem(getPricingSheetWorkspaceStorageKey(), JSON.stringify(fieldnames));
    } catch (e) {
...
function setPricingSheetWorkspaceColumns(frm, fieldnames) {
    ...
    const allowed = new Set(getPricingSheetWorkspaceAllColumns(frm).map((column) => column.fieldname));
    const seen = new Set();
    const normalized = fieldnames.filter((fieldname) => {
        if (!allowed.has(fieldname) || seen.has(fieldname)) {
            return false;
        }
        seen.add(fieldname);
        return true;
...
function movePricingSheetWorkspaceColumnByOffset(frm, fieldname, offset) {
    const columns = [...getPricingSheetWorkspaceColumns(frm)];
    const fromIndex = columns.indexOf(fieldname);
    if (fromIndex === -1) return false;

    const toIndex = Math.max(0, Math.min(columns.length - 1, fromIndex + offset));
...
function renderPricingSheetWorkspaceColumnPopover(frm) {
    const allColumns = getPricingSheetWorkspaceAllColumns(frm);
    const selectedColumns = getPricingSheetWorkspaceColumns(frm);
    const selectedSet = new Set(selectedColumns);
    const orderedColumns = getPricingSheetWorkspaceOrderedColumns(frm);

    return `
        <details class="ps-workspace-column-popover ${Boolean(frm.__ps_workspace_columns_open) ? "is-open" : ""}" ${Boolean(frm.__ps_workspace_columns_open) ? "open" : ""} data-ps-column-details="1">
            <summary>${__("Columns")}</summary>
            <div class="ps-workspace-column-order-title">${__("Display Order")}</div>
            <div class="ps-workspace-column-order-list" data-ps-order-list="1">
                ${orderedColumns.map((column) => `
                    <div class="ps-workspace-column-order-item" data-ps-order-item="${column.fieldname}">
                        <button class="ps-workspace-drag-handle" type="button" data-ps-order-handle="${column.fieldname}" aria-label="${escapePricingSheetText(__(`Drag ${column.label}`))}">::</button>
                        <span class="ps-workspace-column-order-label">${escapePricingSheetText(__(column.label))}</span>
                        <div class="ps-workspace-column-order-actions">
                            <button class="ps-workspace-column-move-btn" type="button" data-ps-move-column="${column.fieldname}" data-ps-move-direction="-1">&uarr;</button>
                            <button class="ps-workspace-column-move-btn" type="button" data-ps-move-column="${column.fieldname}" data-ps-move-direction="1">&darr;</button>
                        </div>
                    </div>
                `).join("")}
            </div>
            <div class="ps-workspace-column-grid">
                ${allColumns.map((column) => {
                    const checked = selectedSet.has(column.fieldname) ? "checked" : "";
                    return `
                        <div class="ps-workspace-column-option">
                            <label>
            ...
            <div class="ps-workspace-empty-state">
                ...
                <div class="ps-workspace-footer">
                    ...
                    <div class="ps-workspace-footer-actions">
                        <div class="ps-workspace-column-menu">${renderPricingSheetWorkspaceColumnPopover(frm)}</div>
...
function renderPricingSheetWorkspace(frm) {
    ...
    $root.find("[data-ps-column-details]").on("toggle", (event) => {
        frm.__ps_workspace_columns_open = Boolean(event.currentTarget.open);
    });
    ...
    bindPricingSheetWorkspaceColumnOrdering(frm, $root);
...
function ensurePricingSheetStyles(frm) {
    ...
    const expected = `/assets/orderlift/css/pricing_sheet_20260414_82.css?${version}`;
...
function applyFormLayoutClass(frm) {
    ...
    forcePricingSheetFullWidthLayout(frm);
}

function forcePricingSheetDeskFullWidth() {
    try {
        window.localStorage.container_fullwidth = "true";
    } catch (e) {
        // Ignore browser storage failures and still force the class.
    }
    ...
    if (frappe && frappe.ui && frappe.ui.toolbar && typeof frappe.ui.toolbar.set_fullwidth_if_enabled === "function") {
        frappe.ui.toolbar.set_fullwidth_if_enabled();
    }
}

function bindPricingSheetWorkspaceColumnOrdering(frm, $root) {
    $root.find("[data-ps-move-column]").on("click", (event) => {
        const fieldname = event.currentTarget.getAttribute("data-ps-move-column");
        const direction = Number(event.currentTarget.getAttribute("data-ps-move-direction") || 0);
        if (!fieldname || !direction) return;
        if (!movePricingSheetWorkspaceColumnByOffset(frm, fieldname, direction)) return;
        frm.__ps_workspace_columns_open = true;
        renderPricingSheetWorkspace(frm);
    });

    const $orderList = $root.find("[data-ps-order-list]");
    if (!$orderList.length || typeof window.PointerEvent === "undefined") {
        return;
    }
    ...
    const cleanupDrag = () => {
        ...
        dragState.list.classList.remove("is-drag-active");
        document.body.classList.remove("ps-workspace-dragging");
    ...
    const handlePointerMove = (event) => {
        if (!dragState) return;
        const target = document.elementFromPoint(event.clientX, event.clientY)?.closest("[data-ps-order-item]");
        if (!target || target === dragState.item || !dragState.list.contains(target)) {
            return;
        }
        ...
        if (referenceNode !== dragState.item) {
    ...
    const handlePointerEnd = () => {
        if (!dragState) return;
        const ordered = Array.from(dragState.list.querySelectorAll("[data-ps-order-item]"))
            .map((node) => node.getAttribute("data-ps-order-item"))
            .filter(Boolean);
        cleanupDrag();
        setPricingSheetWorkspaceColumns(frm, ordered);
        frm.__ps_workspace_columns_open = true;
        renderPricingSheetWorkspace(frm);
    };

    $root.find("[data-ps-order-handle]").on("pointerdown", (event) => {
        if (event.button !== 0) return;
        const item = event.currentTarget.closest("[data-ps-order-item]");
        if (!item) return;
        ...
        document.body.classList.add("ps-workspace-dragging");
...
function forcePricingSheetFullWidthLayout(frm) {
    if (!frm || !frm.$wrapper) return;

    const $mainSection = frm.$wrapper.closest(".layout-main-section");
    if ($mainSection && $mainSection.length) {
        $mainSection.css({
            maxWidth: "none",
            width: "100%",
    ...
    const $mainWrapper = $mainSection && $mainSection.length
        ? $mainSection.closest(".layout-main-section-wrapper")
        : $();
    if ($mainWrapper && $mainWrapper.length) {
        $mainWrapper.css({
            maxWidth: "100%",
        ...
        $mainWrapper.children(".page-body").css({
            maxWidth: "100%",
            width: "100%",
        });
    }

    frm.$wrapper.closest(".std-form-layout").css({
        maxWidth: "100%",
        width: "100%",
    });
    frm.$wrapper.find(".form-layout, .form-page").css({
        maxWidth: "100%",
        width: "100%",
    });
}

function enforcePricingSheetWorkspaceLayout($root) {
    if (!$root || !$root.length) return;

    $root.css({ width: "100%" });
    $root.find(".ps-workspace-shell").css({
        width: "100%",
        overflow: "visible",
    });
    $root.find(".ps-workspace-table-wrap").css({
        width: "100%",
        overflowX: "auto",
        overflowY: "visible",
        position: "relative",
    });
    $root.find(".ps-workspace-link-host, .ps-workspace-link-host .control-input-wrapper, .ps-workspace-link-host .awesomplete, .ps-workspace-link-host input").css({
        width: "100%",
    });
    $root.find(".ps-workspace-link-host .awesomplete").css({ position: "relative" });
    $root.find(".ps-workspace-link-host .awesomplete ul").css({
        position: "absolute",
        zIndex: 1050,
        minWidth: "300px",
        maxWidth: "420px",
...
function applyNativeLinesGridVisibility(frm) {
    ...
    field.$wrapper.addClass("ps-native-grid-hidden");
}
