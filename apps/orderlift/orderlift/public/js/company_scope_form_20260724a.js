 ...
 * here we only add company set+lock for the CRM masters and native transactions.
 */
...
(function () {
    ...
    const SCOPE = {
        ...
        Prospect: { company: "custom_company", bt: null },
        Lead: { company: "custom_company", bt: null },
        ...
        "Delivery Note": { company: "company", bt: null },
        "Material Request": { company: "company", bt: null },
        "Payment Entry": { company: "company", bt: null },
        "Purchase Invoice": { company: "company", bt: null },
        "Purchase Order": { company: "company", bt: null },
        "Purchase Receipt": { company: "company", bt: null },
        "Request for Quotation": { company: "company", bt: null },
        "Sales Invoice": { company: "company", bt: null },
        "Sales Order": { company: "company", bt: "custom_crm_business_type" },
        "Stock Entry": { company: "company", bt: null },
        "Stock Planning Settings": { company: "company", bt: null },
        "Stock Demand Plan": { company: "company", bt: null },
    };
    ...
    function applyCompany(frm, config) {
        ...
        const canOverride = canOverrideCompany();
        if (frm.is_new() && (!frm.doc[field] || (!canOverride && frm.doc[field] !== activeCompany()))) {
        ...
        if (frm._orderlift_company_unlocked) {
            ...
            if (canOverride && Number(frm.doc.docstatus || 0) === 0) addUnlockButton(frm, config);
        }
    }

    function addUnlockButton(frm, config) {
        if (["Lead", "Prospect", "Customer"].includes(frm.doctype)) return;
